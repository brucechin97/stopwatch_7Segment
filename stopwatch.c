...
int main(void)
{
	...
	{
		switch (state)	// checks which state the
		{
		case 0:			// standby mode
			counter = 0;
			display(0, 0, 0);
		...
		case 1:			// start -> stopwatch run mode
			...
			break;
		case 2:			// stop and display stopped time
			display(value_digit4, value_digit3, value_digit2);
			break;
		default:		// to prevent shadow states (states that were not defined)
			state = 0;
		}
	}
	return 0;
